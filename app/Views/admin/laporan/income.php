...
<?= $this->section('title') ?>Income<?= $this->endSection('title') ?>
<?= $this->section('css') ?>
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="modalCenterTitle">Uang Masuk</h5>
                    ...
                    <div class="modal-footer">
                        ...
                        <button id="simpanData" type="button" class="btn btn-primary">Tambahkan</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal fade" id="editData" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="modalCenterTitle">Ubah Uang Masuk</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="row">
                        ...
                        <div class="row g-2 mb-3">
                            ...
                            <div class="col mb-0">
                                <label for="dobWithTitle" class="form-label">Jenis</label>
                                <select id="jenis" class="type selectpicker w-100" data-style="btn-default">
                                    <option value="DEPOSIT">Deposit</option>
                                    <option value="PELUNASAN">Pelunasan</option>
                                    <option value="LAINNYA">Lainnya</option>
                                </select>
                    ...
                    <div class="modal-footer">
                        ...
                        <button id="updateData" type="button" class="btn btn-success">Update</button>
                    </div>
                </div>
            </div>
        </div>
...
$(function() {
    ...
    if (dt_basic_table.length) {
        dt_basic = dt_basic_table.DataTable({
            ...
            columnDefs: [{
                ...
                },
                {
                    targets: 4,
                    render: function(data, type, full, meta) {
                        var nominal = full['income_nominal'];
                ...
                },
                {
                    targets: 5,
                    render: function(data, type, full, meta) {
                        var $status_number = full['income_type'];
                        var $status = {
                            "DEPOSIT": {
                                title: 'Deposit',
                ...
                },
                {
                    ...
                    searchable: false,
                    render: function(data, type, full, meta) {
                        var $income_id = full['income_id'];
                        var $tanggal = full['income_created'];
                        var $nominal = full['income_nominal'];
                        var $type = full['income_type'];
                        var $desc = full['income_desc'];
                        var $actor = full['income_actor'];
                        return (
                            '<a onclick="editData(\'' + $income_id + '\',\'' + $tanggal +
                            '\',\'' + $nominal + '\',\'' + $type + '\',\'' + $desc +
                            '\')" href="javascript:;" class="btn btn-sm btn-icon item-edit"><i class="text-primary ti ti-pencil"></i></a>' +
                            '<a onclick="deleteData(\'' + $income_id +
        ...
        $('div.head-label').html(
            '<div class="btn-group" role="group" aria-label="Basic example"><button type="button" class="btn btn-primary"  data-bs-toggle="modal" data-bs-target="#tambahData"><i class="ti ti-plus me-sm-1"></i><span class="d-none d-sm-inline-block">Tambah Uang Masuk</span></button><button id="refreshtable" type="button" class="btn btn-primary"><i class="ti ti-refresh me-sm-1"></i></button></div>'
        );
        refreshData();
    ...
    $('#updateData').on('click', function() {
        var id = $('#idincome').val(),
            tgl = $('.tgl').val(),
            nom = $('.nom').val(),
            type = $('#jenis').find("option:selected").val(),
            desc = $('.desc').val(),
            actor = "<?= $session->get('admin_fullname'); ?>";
        $.ajax({
            type: "POST",
            url: "<?= base_url('updateUangMasuk') ?>",
            data: {
                id: id,
                tgl: tgl,
                nom: nom,
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                var income = parseInt(response.success.all),
                    outcome = parseInt(response.success.out);

                if (outcome > income) {
                    formattedClear = "- " + formatRupiah(response.success.clear);
                }
...
function deleteData(id) {
    $.ajax({
        type: "GET",
        url: "<?= base_url('deleteUangMasuk') ?>/" + id,
        data: "",
        dataType: "json",
        success: function(response) {
            if (response.success) {
